...
class BasicAPIWrapperMCP {
  ...
  private setupToolHandlers(): void {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case "manage_task":
            return await this.handleTaskManagement(args as unknown as TaskOperation);
          case "get_team_overview":
            return await this.handleTeamOverview(args as TeamOverview);
          case "get_weather":
            return await this.handleWeather(args as WeatherArgs);
          default:
  ...
  private async handleTaskManagement(args: TaskOperation) {
      ...
      switch (operation) {
        ...
        case "update": {
          ...
          const response = await axios.patch(
            `${this.baseUrl}/tasks/${args.task_id}`,
            payload,
            { headers }
          );
          const task = response.data;
  ...
  private async handleTeamOverview(args: TeamOverview) {
    try {
      const headers = {
        Authorization: `Bearer ${await this.authManager.getToken()}`,
        "Content-Type": "application/json",
  ...
  private async handleWeather(args: WeatherArgs) {
